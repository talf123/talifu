          ...
          <div class="top">
            <label>用户idx：</label>
              ...
              <Select v-model="query" size="small" style="width:150px; margin-left: 10px;">
                  <Option v-for="item in recgift" :value="item.value" :key="item.value">{{ item.label }}</Option>
              </Select>
              <Button type="primary" size="small" @click="rec">礼物热推添加</Button>
          ...
          <div class="center" >
            <Table border :columns="columns" :data="data" class="table"  ref="table">
                ...
                <template slot-scope="{ row, index }" slot="action">
                    <Button type="text" size="small" @click="remove(index,row)">黑名单</Button>
                </template>
            </Table>
          </div>
          <!-- 遮罩层 -->
  ...
  data () {
    return {
        ...
        columns: [
            ...
            {title: '房间ID',key: 'id',width:150}, 
            {title: '主播ID',key: 'money',width:150},
            {title: '昵称',key: 'volue',width:150},
            {title: '视频',key: 'gift',width:200},
            {title: '图像',key: '',width:150},
            {title: '礼物',key: 'address',width:120},
            {title: '操作',slot: 'action',align: 'center',width:107}],
        data: [ // 数据表数据
            {
                pay: 'Android', // 支付类型
                id: 'com.sundady.80', // 商品ID
                money: 1, // 金额
                volue: 7000, //  币值
                gift: 0, // 礼物赠送币
