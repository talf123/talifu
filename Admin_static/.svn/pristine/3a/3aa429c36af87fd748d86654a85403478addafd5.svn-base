            ...
            <div class="center" >
                <Table border :columns="columns" :data="tabledata" class="table"  ref="table">
                    ...
                    <template slot-scope="{ row, index }" slot="action">
                        <Button type="text" size="small" style="margin-right: 5px" @click="update(row)">锁定头像</Button>
                        <Button type="text" size="small" @click="remove(index,row)">明细</Button>
                    </template>
                </Table>
            </div>
          <!-- 遮罩层 -->
  ...
  data () {
    return {
        ...
        columns: [
            ...
            {title: '昵称',key: 'money'},
            {title: '性别',key: 'volue'},
            {title: 'VIP等级',key: 'gift'},
            {title: '金币',key: 'address'},
            {title: '签名',key: 'address'},
            {title: '小图',key: ''},
            {title: '注册时间',key: 'address'},
            {title: '最后登录时间',key: 'address'},
...
.main{  // 主页面样式
    ...
    .top{margin-top:20px; margin-left: 20px; display: flex; font-size: 20px;  // 表头部分 查找添加 样式
          button{
              margin-left: 15px;
          }
          label{
    ...
    .center{width:100%; margin-top:20px;}  // 页面中部 表格部分样式
