        ...
        <div class="main" :style="widthMain">
          ...
          <div>
            <div class="top">
                ...
                <label>时间选择：</label>
                <DatePicker type="date" size="small" placeholder="Select date" style="width: 200px"></DatePicker>
                <label>对比日期：</label>
                <DatePicker type="date" size="small" placeholder="Select date" style="width: 200px"></DatePicker>

                <Button type="primary" size="small" @click="">当天查询</Button>
                <Button type="primary" size="small" @click="">两天对比</Button>
                <Button type="primary" size="small" @click="datalookup">列表数据查询</Button>
            </div>
            <div class="top">
                <span>{{}}</span><label>高峰人气：</label><span>{{}}</span><label>人</label><span>{{}}</span><label>高峰人气：</label><span>{{}}</span><label>人</label><label>高峰比：</label><span>{{}}</span>
            </div>
            <div class="top">
                <span>{{}}</span><label>平均人气：</label><span>{{}}</span><label>人</label><span>{{}}</span><label>平均人气：</label><span>{{}}</span><label>人</label><label>平均人气比：</label><span>{{}}</span>
            </div>
            <div class="top">
                <span>{{}}</span><label>当前人气：</label><span>{{}}</span><label>人</label><span>{{}}</span><label>当前人气：</label><span>{{}}</span><label>人</label><label>当前在线人气比：</label><span>{{}}</span>
            </div>
            <div class="top">
                <span>{{}}</span><label>当前主播在线：</label><span>{{}}</span><label>人</label><span>{{}}</span><label>当前主播在线：</label><span>{{}}</span><label>人</label>
            </div>
          </div>
          <!-- 主页面内容 -->
          <div class="center" >
          ...
          <!-- 新增页面 -->
            <div class="pop" v-if="showNonecreate" >
                <div style="display:flex; margin: 15px;">
                    <label style="font-size: 15px;">时间选择：</label>
                    <DatePicker type="daterange" size="small" split-panels placeholder="Select date" style="width: 200px"></DatePicker>
                    <Button type="primary" size="small" @click="" style="margin: 0px 10px 0px 10px;">查询</Button>
                    <Button type="primary" size="small" @click="close">折线图</Button>
                </div>
                <div style="display:flex; font-size: 15px; margin: 15px;">
                    <label>平均值：</label>
                </div>
...
export default {
  ...
  data () {
    return {
        ...
        columns: [
            {title: '时间',key: 'pay'},
            {title: '平台在线峰值',key: 'id'}, 
            {title: '平台平均在线',key: 'money'},
            {title: '房间在线峰值',key: 'volue'},
            {title: '房间平均在线',key: 'gift'}
        ],
        data: [ // 数据表数据
            {
                pay: 'Android', // 支付类型
  ...
  methods: {
      ...
      datalookup: function () {
        this.showNone = true, // 控制页面遮罩层开关
        this.showNonecreate = true // 新增页面弹出控制
      },
      close: function () {
          this.showNone = false,
