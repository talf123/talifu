          ...
          <div class="center" >
            <Table border :columns="biao" :data="data" class="table"  ref="table">
                ...
                <template slot-scope="{ row, index }" slot="action">
                    <Button type="text" size="small" style="margin-right: 5px" @click="update(row)">通过</Button>
                    <Button type="text" size="small" @click="remove(index,row)">驳回</Button>
                </template>
            </Table>
          </div>
          <!-- 遮罩层 -->
  ...
  data () {
    return {
        ...
        columns1: [
            ...
            {title: '有无直播经验',key: 'address'},
            {title: '照片',key: 'address'},
