          ...
          <!-- 查询 -->
            <div class="top">
                <label>时间范围：</label>
                    ...
                    <Button type="primary" size="small" @click="lookup">当月</Button>
                    <Button type="primary" size="small" @click="lookup">上月</Button>
            </div>
            <div style="display:flex; font-size:15px; margin-left: 20px; margin-top: 15px;">
                <span v-for="item in total" :key="item.value" :value="item.value">
                    <label>{{item.label}}：</label><span style="color:red; margin-right: 10px;">{{item.sum}}</span>
                </span>
            </div>
          <!-- 主页面内容 -->
          <div class="center" >
